...
%                   [Covertree_distancefcn]         : distance function to use in covertree.
%                   [Covertree_classname]           : classname to use in covertree.
%               
%com
% OUT:
...
if isfield(opts,'ReuseOpts') && (opts.ReuseOpts),
    ...
    if ~isfield(opts,'Covertree_distancefcn'),  opts.Covertree_distancefcn = int32(0);                          end
    if ~isfield(opts,'Covertree_classname'),    opts.Covertree_classname = int32(0);                            end
    if ~isfield(opts,'CoverTreeOpts') || isempty(opts.CoverTreeOpts)         
        opts.CoverTreeOpts = struct('theta',0.5,'numlevels',int32(100),'minlevel',int32(0), 'BLOCKSIZE',int32(1024), ...
                                        'distancefcn',opts.Covertree_distancefcn, 'classname', opts.Covertree_classname);
    end
end
