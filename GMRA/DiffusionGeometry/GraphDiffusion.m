function G = GraphDiffusion( X, Radius, Options )
...
% IN:
%
%    X                      : DxN matrix specifying N points in R^D
...
%         'beltrami'            Laplace-Beltrami normalization a la Coifman-Lafon
...
%                               'W+Wt'     : replaces W by W+Wt. This is the default.
...
%      [Subsampling]        : structure with options about how to subsample the data
%                               [Type] : string in {'none','random','net'}. Default: 'none'.
%                               [Size] : (approximate) size of the subsample, It will randomly subsample the currently subsampled set. Default: Inf
%                               [Radius] : radius of net, if Type=='net'.
%                               [MAXpts] : maximum number of points in the subsamlped set.
%
...
% Example:
%   X=[ones(5,256),-ones(5,256)];X=X+0.1*randn(size(X));
%   G = GraphDiffusion(X, 0, struct('kNN',10,'kEigenVecs',10,'CoverTreeOpts',struct('distancefcn',COVERTREE_DISTANCE_EUCLIDEAN_ABS_VALUES,'classname',COVERTREE_CLASSNAME_VECTOR)));
% Example with downsampling
%   X = rand(2,10000).^2;X(1,:)=2*X(1,:);
%   G = GraphDiffusion(X, 0.5, struct('Epsilon',sqrt(0.1),'Normalization','smarkov','kEigenVecs',100));
%   G2 = GraphDiffusion(X,0.5,struct('Epsilon',sqrt(0.1),'Normalization','smarkov','kEigenVecs',100,'Subsampling',struct('Type','net','Radius',0.02,'Size',Inf)));
%   figure;plot(G.EigenVals,'.-');hold on;plot(G2.EigenVals,'r.-');figure;imagesc((abs(G.EigenVecs(G2.SubS,:)'*G2.EigenVecs)));colorbar;
%
...
G.Opts.Subsampling      = struct('Type','none','Size',Inf);
...
for j=1:length(fn)
    ...
    elseif strcmpi(name,'Subsampling')      G.Opts.Subsampling = value;
...
G.SubS = [];
G.Sweights = [];
if strcmpi(G.Opts.Subsampling.Type,'random')
   G.SubS     = randperm(N,G.Opts.Subsampling.Size);
   G.Sweights = N/G.Opts.Subsampling.Size*ones(length(SubS),1);
elseif strcmpi(G.Opts.Subsampling.Type,'net')
   [G.SubS,G.Sweights] = ConstructNet(X,G.Opts.Subsampling.Radius);%,struct('weightfcn',@(x) exp(-x.^2/G.Opts.Epsilon^2)));
   if length(G.SubS)>G.Opts.Subsampling.Size
       G.SubS0    = G.SubS;
       G.SubS     = G.SubS(randperm(length(G.SubS),G.Opts.Subsampling.Size));
   end
end
if ~isempty(G.SubS)
    X = X(:,G.SubS);
    ...
    if isempty(DistInfo) || ~isfield(DistInfo,'idxs')
        ...
        if isfield(DistInfo,'CoverTree')
            ...
            if (G.Opts.NNMaxDim>0) && (G.Opts.NNMaxDim<Dim) && (G.Opts.NNMaxDim<N),
                lPointsProj = X;
                lPointsProj = bsxfun(@minus,X,mean(lPointsProj,2));
        ...
        if G.Opts.DoRadiusSearch       % Do radius search
            [DistInfo.count, DistInfo.idxs, DistInfo.dists,DistInfo.NNInfo] = nrsearch(X, [], [], max([G.Opts.Radius,G.Opts.PCARadius]), lOptsTemp);
        else                            % Do nearest neighbors search
            [DistInfo.count, DistInfo.idxs, DistInfo.dists,DistInfo.NNInfo] = nrsearch(X, [], max(G.Opts.kNN), [], lOptsTemp);
...
for k = 1:lNumberOfGraphs
    if isempty(G.Opts.W)
        ...
        if ~isempty(strfind(G.Opts.Distance,'PCA'))
            ...
            for i = N:-1:1
                ...
                if G.Opts.DoRadiusSearch
                    ...
                    pca_idxs  = DistInfo.idxs{i}(DistInfo.dists{i}<=G.Opts.PCARadius(k));
                ...
                % Center the points
                lCenteredPcaPts = bsxfun(@minus,X(:,pca_idxs),-X(:,i));
                % Compute local PCA
                ...
                lCenteredRadiusPts = bsxfun(@minus,X(:,idxs{i}),X(:,i));
        ...
        if isempty(G.Opts.WeightFcn)
            ...
            for k = 1:N
                ...
                if ~isempty(lIdxs)
                    lW = feval( G.Opts.WeightFcn, X, [k,lIdxs], [] );
    ...
    if isempty(G.SubS)
        ...
        DInv = sum(W*spdiags(G.Sweights,[0],size(W,1),size(W,2)),2);
    end
    ...
    if ~isempty(G.SubS)
        T = T*spdiags(G.Sweights,[0],size(T,1),size(T,2));
    end
