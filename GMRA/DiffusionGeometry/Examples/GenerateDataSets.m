function [X,Labels,OutParams] = GenerateDataSets( cDataName, cOpts )
...
%                   'Caltech101'      : images from Caltech 101 database.
%                           cOpts.Labels : string array of names of the classes to load.
%                           cOpts.SamplesPerClass : number of samples per class to collect (not guaranteed depending on training set available).
%
...
%                   [AutotuneScales]  : compute set of scales for the data set to go in OutParams.NetsOpts. Default: true if OutParams.NetsOpts is an output.
...
%   OutParams     : Data structure with the following fields:
%                   GraphDiffOpts : structure of options for GraphDiffusion.
%                   NetsOpts      : structure of options for FastRoughMultiscaleSVD.
%                   LoadImageBatchParams : strucutre of parameters for LoadImageBatchParams call when applicable.
%                   Coords        : Coordinates for X. For example if SVDProject is specified, OutParams.Coords is the basis (each basis element in a column) onto which X was projected.
...
if ~isfield(cOpts,'MeyerStepWidth'),        cOpts.MeyerStepWidth = 0.2;         end
...
OutParams.GraphDiffOpts = struct( ...
...
switch lower(cDataName)
    ...
    case {'d-sphere'}
        ...
        Xo = Xo';
        OutParams.GraphDiffOpts.kNN = min([cOpts.Dim*10,cOpts.NumberOfPoints]);
        OutParams.GraphDiffOpts.kNNAutotune = max([ceil(OutParams.GraphDiffOpts.kNN/10),5]);
    case {'sphereandline'}
        lOpts = cOpts;
        lOpts.NumberOfPoints = round(2/3*cOpts.NumberOfPoints);
        lOpts.Dim = 2;
        lOpts.EmbedDim = 3;
        [X_1,Labels,OutParams] = GenerateDataSets( 'D-Sphere',lOpts );
    ...
    case {'s-manifold'}
        ...
        Xo = Xo';
    case {'z-manifold'}
        Xo = Generate_Z_Manifold(cOpts.NumberOfPoints,cOpts.Dim);
        Xo = Xo';
    case {'curved-z-manifold'}
        Xo = Generate_CurvedZ_Manifold(cOpts.NumberOfPoints,cOpts.Dim);
        Xo = Xo';
    ...
    case {'swissroll'}
        ...
        Xo = Xo';
    case {'swissrollskew'}
        Xo = Generate_SwissrollSkew(cOpts.NumberOfPoints,cOpts.Dim);
        Xo = Xo';
    case {'spiralandplane'}
    ...
    case {'meyerstaircase-d'}        
        if ~isfield(cOpts,'MeyerStepNorm')      cOpts.MeyerStepNorm = 2;                end
        if ~isfield(cOpts,'avoidbdry')          cOpts.avoidbdry = 0;                    end
        if ~isfield(cOpts,'thres')              cOpts.thres = 0;                        end
        if ~isfield(cOpts,'EmbedDim')           cOpts.EmbedDim = 0;                     end
        
        nptsperdim          = round(cOpts.NumberOfPoints^(1/cOpts.Dim));
        cOpts.NumberOfPoints= nptsperdim^(cOpts.Dim);
        if cOpts.EmbedDim == 0
            nticksperdim    = round(cOpts.NumberOfPoints^(1/cOpts.Dim));
        else
            nticksperdim    = round(cOpts.EmbedDim^(1/cOpts.Dim));
            cOpts.EmbedDim  = nticksperdim^(cOpts.Dim);
        end
        ptsperdim  = linspace(-1,1,nticksperdim);
        
        if ~isfield(cOpts,'width'),             cOpts.width = 1/(nptsperdim/3);         end
        ...
        for k = 1:cOpts.Dim,
            ...
            for j = 1:cOpts.Dim,
                str_eval = [str_eval,num2str(nptsperdim),','];
            end
            str_eval(end) = ',';
            str_eval = [str_eval,'1)-0.5)*2*(1-',sprintf('%f',cOpts.avoidbdry),');'];
            eval(str_eval);
        end
        
        % Construct Gaussians
        if cOpts.EmbedDim == 0
            Xo  = zeros(cOpts.NumberOfPoints,prod(size(Xs_1)));
        else
            Xo  = zeros(cOpts.NumberOfPoints,cOpts.EmbedDim);
        end
        OutParams.Coords  = zeros(size(Xo,1),cOpts.Dim);
        for i = 1:size(Xo,1)                                                                                                    % Create the coordinates of the centers
            str_eval    = 'OutParams.Coords(i,:) = [';
            for k = 1:cOpts.Dim,
        ...
        if isnumeric(cOpts.width)
            parfor i = 1:size(Xo,1)                                                                                                 % Create the actual Gaussians
                tmp             = CreateGriddedGaussian(cOpts.Dim,ptsperdim,OutParams.Coords(i,:),cOpts.width^2, cOpts.MeyerStepNorm);
                Xo(i,:)         = tmp(:)/max(abs(tmp(:)));
            end
        else
            lNormsSq            = sum(OutParams.Coords.^2,2);
            lMaxNormsSq         = max(lNormsSq)*(1.01);
            for i = 1:size(Xo,1)                                                                                                 % Create the actual Gaussians
                widthSq         = lNormsSq(i)^3+1e-4;
                tmp             = CreateGriddedGaussian(cOpts.Dim,ptsperdim,OutParams.Coords(i,:).^5,widthSq, cOpts.MeyerStepNorm);
                Xo(i,:)         = tmp(:)/norm(tmp,cOpts.MeyerStepNorm);
            end
        end
        Labels = OutParams.Coords;
        AutoScalesMinNPerBin = 10;
        Xo = Xo';
        if cOpts.thres>0
            for i = 1:size(Xo,2)
                Xo(:,i) = Xo(:,i).*(Xo(:,i)>cOpts.thres*max(Xo(:,i)));
            end
        end
    case {'meyerstaircase'}
        ...
        AutoScalesMinNPerBin = 10;
        OutParams.GraphDiffOpts.kNN = 5;
        OutParams.GraphDiffOpts.kNNAutotune = 3;
        Xo = Xo';
    case {'bmark_g241c'}
    ...
    case {'bmark_mnist'}
        ...
        OutParams.GraphDiffOpts.imgSize = [28,28];
        Xo = Xo';
    case {'fromimage'}
        try
    ...
    case {'mm_faces'}
        ...
        Xo=double(Xo);
    case {'caltech101'}
        if ~isfield(cOpts,'Labels') || isempty(cOpts.Labels)                    cOpts.Labels = {'airplanes','bonsai','emu'};end
        if ~isfield(cOpts,'SamplesPerClass') || isempty(cOpts.SamplesPerClass)  cOpts.SamplesPerClass = 50;                 end
        
        % Set parameters for data loading and preparation
        OutParams.DataSetName                               = 'Caltech 101';
        OutParams.LoadImageBatchParams.Categories           = cOpts.Labels;
        OutParams.LoadImageBatchParams.ImagesPerCategory    = cOpts.SamplesPerClass;
        OutParams.LoadImageBatchParams.Scales               = [1,2,4,8,16];
        OutParams.LoadImageBatchParams.PatchSz              = [16,16];
        OutParams.LoadImageBatchParams.PatchTau             = [4,4];
        OutParams.LoadImageBatchParams.TrainFraction        = 1;
        OutParams.Whiten                                    = true;
        OutParams.AbsFFT                                    = true;
        OutParams.GraphDiffOpts.imgSize                     = OutParams.LoadImageBatchParams.PatchSz;
        
        [Images, Labels, Xo, OutParams.MultiscalePatchInfo] = LoadImageBatch( OutParams.DataSetName, OutParams.LoadImageBatchParams );
        Labels                                              = OutParams.MultiscalePatchInfo.labelidx;        
        Xo                                                  = single(Xo);   
        normidxs                                            = find(sum(Xo.^2,1)>1e-3);
        Xo                                                  = Xo(:,normidxs);
        OutParams.MultiscalePatchInfo.coords                = OutParams.MultiscalePatchInfo.coords(:,normidxs);
        OutParams.MultiscalePatchInfo.imageidx              = OutParams.MultiscalePatchInfo.imageidx(:,normidxs);
        OutParams.MultiscalePatchInfo.labelidx              = OutParams.MultiscalePatchInfo.labelidx(:,normidxs);
        OutParams.MultiscalePatchInfo.training              = OutParams.MultiscalePatchInfo.training(:,normidxs);
        if OutParams.Whiten,                             Xo = Whiten( Xo ); end                                                                             
        if OutParams.AbsFFT,                             Xo = AbsFFT( Xo, OutParams.LoadImageBatchParams.PatchSz ); end
        
    case {'patchesfromimage'}
        % Set parameters
        OutParams.DataSetName                               = cOpts.FileName;
        OutParams.LoadImageBatchParams.Categories{1}        = cOpts.FileName;
        OutParams.LoadImageBatchParams.Scales               = [1,2,4,8,16,32,64];
        OutParams.LoadImageBatchParams.PatchSz              = [8,8];
        OutParams.LoadImageBatchParams.PatchTau             = [1,1];
        OutParams.LoadImageBatchParams.TrainFraction        = 1;
        OutParams.Whiten                                    = true;
        OutParams.AbsFFT                                    = true;
        OutParams.GraphDiffOpts.imgSize                     = OutParams.LoadImageBatchParams.PatchSz;
    ...
    case {'music'}
        opts.Downsample = 8;
        opts.WindowSizeInSeconds = 0.1;
        opts.WindowStepInSeconds = 0.005;
        opts.Normalization = 'Linfty';
        opts.Transform = 'absfft'; %'rceps';
        opts.TimeWindowInSec = [10,1200];
        
        %Xo = LoadMusicData( '06 Variation 5 a 1 ovvero 2 Clav.wav', opts );
        Xo = LoadMusicData( '01 Sonata Kreutzer.m4a', opts );
        
    case {'puma'}
        njoints = (cOpts.Dim+1)/3;
        [Xo,Labels] = puma(njoints,cOpts.NumberOfPoints);
        Xo = Xo';
        cOpts.EmbedDim = cOpts.Dim;
...
if cOpts.SVDProject>0,
    ...
    [lU,~,~] = svd(Xo,0);
    OutParams.Coords = lU(:,1:cOpts.SVDProject);
    Xo = OutParams.Coords'*Xo;
    OutParams.NetsOpts.Delta = OutParams.NetsOpts.Delta*sqrt(cOpts.SVDProject/cOpts.Dim);
...
if ~isempty(cOpts.RandomDilationFactors),
    ...
    OutParams.NetsOpts.RandomDilationFactorsRealization = lRandDilations;
end
...
if ~isempty(cOpts.RandomScaling),
    ...
    X = lRandDilation*X;
    OutParams.NetsOpts.RandomScalingRealization = lRandDilation;
end
...
if nargout>2
    ...
    if cOpts.AutotuneScales
        if size(X,2)<10000,
            OutParams.NetsOpts.Delta = AutoScales(X,struct('MinNperBin',min([min(size(X,1)),AutoScalesMinNPerBin]),'StatType','Min'));
            OutParams.NetsOpts.NumberOfScales = length(OutParams.NetsOpts.Delta);
        else
            warning('Autotunescales not computed: may take too long');
        end
    end
end

OutParams.GenerateDataSets.Opts = cOpts;
