function [U,S,V] = randKPCA(A,k,its,l,K)
...
%   Copyright 2009 Mark Tygert. Modified version, and kernelization, by M. Maggioni
...
if nargin>=5 && ~isempty(K)
    A_kernel = K(A,A);
    A_tmp = bsxfun(@minus,A_kernel,sum(A_kernel,1)/n);
    A = A_kernel - A_tmp - bsxfun(@minus,A_kernel,sum(A_kernel,2)/n) - mean(A_tmp);
    
    if MEMORY_THRIFTY, clear A_kernel A_tmp; end;
end
