function gMRA = construct_pruningGeometricWavelets( X,gMRA )
...
%%
flags = ones(nAllNets,1); % indicator of whether to keep (1) or remove (-1) a node
...
while j>0 && n>0
    ...
    for i = 1:length(nodes)
        ...
        if numel(children)>1 % multiple children, construct local geometric wavelets and compare among the different encoding costs
            ...
            nodeInfo = local_SVD_analysis(X,gMRA,node,gMRA.PointsInNet{node});
            gMRA.Sizes(node)    = nodeInfo.Size;
            gMRA.Centers{node}  = nodeInfo.Center;
            gMRA.Radii(node)    = nodeInfo.Radius;
            gMRA.Sigmas{node}   = nodeInfo.Sigmas;
            gMRA.ScalFuns{node} = nodeInfo.ScalFuns;
            if isfield(nodeInfo,'epsEncodingCost')   gMRA.epsEncodingCost(node)  = nodeInfo.epsEncodingCost; end
            if isfield(nodeInfo,'Projections')       gMRA.Projections{node}      = nodeInfo.Projections;     end
