...
if (nargin<4) || (isempty(labels)), labels=(0:length(p)-1); end
if (nargin<5) || (isempty(vertexstyle)), vertexstyle = 'r.'; end
...
if (nargin == 1) || (isempty(c)) || ~isempty(labels)
    n = length(p);
    if n < 50000,
        if isempty(c)
        ...
        for k = 1:n,
            if (~isempty(c) && c(k)~=0) || (isempty(c) && labels(k)~=0),
                text(x(k)+0.00,y(k)-0.000,num2str(labels(k)));
            end
            if ~isempty(labels2),
                if labels2(k)~=0,
                    text(x(k)+0.00,y(k)-0.00,num2str(labels2(k)));
                end
            end
        end
    else
